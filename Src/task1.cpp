...
void TASK1::run(void)
{
	...
	case S0_INIT: // initialize choke control
	{
		...
		temp_min = 28;         // min temp to control choke positon [C]
		temp_operating = 35;  // max temp to control choke positon [C]
		...
		position_max = 1100;   // maximum position corresponding to 45 deg [encoder ticks]
	...
	case S1_PAUSE: // wait for user input to begin control
	{
		...
		{
			if (calibrated == false) state = S2_LIMIT; // calibrate if not
	...
	case S2_LIMIT: // calibrate motor position
	{
		if (HAL_GPIO_ReadPin (GPIOG, GPIO_PIN_1) == GPIO_PIN_SET)      // choke control switch on
		{
			...
			if(HAL_GPIO_ReadPin (GPIOG, GPIO_PIN_4) == GPIO_PIN_RESET) // check for limit switch hit
			{
	...
	case S3_RUN: // run choke control
	{
		...
		if (int_err > 100000) int_err = 100000;    // saturate integral error high
		if (int_err < -100000) int_err = -100000;  // saturate integral error low
		...
		char buffer_tx[160];
		sprintf(buffer_tx,
		        "Temp: %d | Setpoint: %d | Measured: %d | Error: %d | Duty: %d | IntErr: %d\r\n",
		        temp_meas, position_ref, position_meas, err, L, int_err);

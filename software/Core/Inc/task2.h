...
public:
	...
	int engine_speed_share;      ///< Engine speed accessible outside of class
	int distance_share;          ///< Total distance traveled accessible outside of class
	GPIO_PinState gear_share[7]; ///< Gear selection array accessible outside of class
...
private:
	...
	uint32_t time_old;             ///< old encoder time [msec]
	uint32_t temp_time_old;        ///< old temp time [msec]
	uint32_t time_new;             ///< new time [msec]
	uint32_t dt;            ///< time difference between encoder readings [msec]
	uint32_t temp_dt;          ///< time difference between temp readings [msec]
	uint32_t enc_time;  ///< minimum time needed between encoder readings [msec]
	uint32_t thermo_time;  ///< minimum time needed between temp readings [msec]
	int wheel_speed;               ///< front wheel speed [MPH]
	int engine_speed;              ///< engine speed [RPM]
	int distance;                  ///< distance traveled [mi]
	float wheel_speed_conversion;  ///< convert encoder ticks / second to MPH
	float engine_speed_conversion; ///< convert encoder ticks / second to RPM
	float distance_conversion;     ///< convert encoder ticks to mi
	float engine_speeds[5];        ///< last 5 speed measurements
